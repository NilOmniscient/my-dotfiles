...
local function filter_apps()
	...
	if search_text == "" then
		...
		for _, app in ipairs(all_apps) do
			...
			if name_score == nil and comment_score == nil then
				load_apps()
				name_score = fuzzy_match(search_text, app.name)
				comment_score = fuzzy_match(search_text, app.comment)
			end
...
local function get_initial_color(name)
	...
	return beautiful.initial_colors[(sum % #beautiful.initial_colors) + 1]
end
